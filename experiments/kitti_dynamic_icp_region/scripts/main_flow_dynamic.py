...
EXP_ROOT = os.path.dirname(_SCRIPT_DIR)
PROJECT_ROOT = os.path.dirname(os.path.dirname(EXP_ROOT))
EXP_UTILS_DIR = os.path.join(EXP_ROOT, "utils")
PROJECT_SCRIPTS_DIR = os.path.join(PROJECT_ROOT, "scripts")

if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)
if PROJECT_SCRIPTS_DIR not in sys.path:
    sys.path.insert(1, PROJECT_SCRIPTS_DIR)
if EXP_UTILS_DIR not in sys.path:
    sys.path.insert(2, EXP_UTILS_DIR)
...
from dynamic_utils import (
    DynamicStepInfo,
    build_dense_depth_observations,
    build_lidar_observations,
    run_icp_dynamic_residuals,
    run_lidar_icp_dynamic_residuals,
    draw_dynamic_overlay,
    draw_paired_depth_visualization,
    write_step_csv,
)
...
def rgbd_slam(config: dict):
    ...
    if "dynamic" not in config:
        raise ValueError("Experiment config must provide a `dynamic` section.")
    if "run_final_eval" not in config:
        config["run_final_eval"] = False
    if "save_final_params" not in config:
        config["save_final_params"] = False
    print(f"{config}")
    ...
    dynamic_prev_dir = os.path.join(output_dir, "vis_prev")
    dynamic_curr_dir = os.path.join(output_dir, "vis_curr")
    dynamic_prev_lidar_dir = os.path.join(output_dir, "vis_prev_lidar")
    dynamic_curr_lidar_dir = os.path.join(output_dir, "vis_curr_lidar")
    depth_pair_dir = os.path.join(output_dir, "vis_depth_pairs")
    os.makedirs(dynamic_prev_dir, exist_ok=True)
    os.makedirs(dynamic_curr_dir, exist_ok=True)
    os.makedirs(dynamic_prev_lidar_dir, exist_ok=True)
    os.makedirs(dynamic_curr_lidar_dir, exist_ok=True)
    os.makedirs(depth_pair_dir, exist_ok=True)
    ...
    dynamic_steps = []
    dynamic_steps_lidar = []
    lidar_to_cam = load_velo_to_cam(dataset, PROJECT_ROOT)
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        curr_rgb_u8 = (torch.clamp(color, 0.0, 1.0).permute(1, 2, 0).detach().cpu().numpy() * 255.0).astype(np.uint8)
        ...
        if time_idx > 0:
            ...
            if not config['use_warp_loss'] or pnp_num_inliers < 10:
                ...
                if pose_init_method == "pnp_fused_icp":
                    ...
                    dynamic_init_transform = np.array(init_pose, dtype=np.float64)
        ...
        if time_idx > 0 and dynamic_init_transform is not None:
            draw_paired_depth_visualization(
                last_data["depth_original"].detach().cpu().numpy(),
                curr_data["depth_original"].detach().cpu().numpy(),
                os.path.join(depth_pair_dir, f"{time_idx:06d}.png"),
                prev_label=f"Prev Depth {last_data['frame_id']}",
                curr_label=f"Curr Depth {curr_data['frame_id']}",
            )
            source_obs = build_dense_depth_observations(
                last_data["depth_original"].detach().cpu().numpy(),
                intrinsics.detach().cpu().numpy(),
                dataset.depth_filter_near,
                dataset.depth_filter_far,
                pixel_stride=config["dynamic"].get("pixel_stride", 1),
            )
            target_obs = build_dense_depth_observations(
                curr_data["depth_original"].detach().cpu().numpy(),
                intrinsics.detach().cpu().numpy(),
                dataset.depth_filter_near,
                dataset.depth_filter_far,
            ...
            last_w2c_np = last_data["est_w2c"].detach().cpu().numpy()
            curr_w2c_np = curr_data["est_w2c"].detach().cpu().numpy()
            optimized_transform = curr_w2c_np @ np.linalg.inv(last_w2c_np)
            pose_source = config["dynamic"].get("pose_source", "fused_init")
            if pose_source == "optimized_pose":
                eval_transform = optimized_transform
            else:
                eval_transform = dynamic_init_transform

            residuals = run_icp_dynamic_residuals(
                source_obs,
                target_obs,
                eval_transform,
                intrinsics.detach().cpu().numpy(),
                curr_data["depth_original"].detach().cpu().numpy(),
                config["dynamic"].get("icp_corr_threshold", config["tracking"].get("icp_corr_threshold", 0.5)),
                config["dynamic"].get("residual_thresh_m", 0.5),
                config["dynamic"].get("unmatched_penalty", 2.0),
                config["dynamic"].get("search_radius_px", 1),
            )

            compared_count = int(residuals["last_uv"].shape[0])
            if compared_count > 0:
                last_uv = residuals["last_uv"]
                curr_uv = residuals["curr_uv"]
                dynamic_mask = residuals["dynamic_mask"]
                static_mask = residuals["static_mask"]
                dynamic_count = int(np.sum(dynamic_mask))
                dynamic_ratio = float(dynamic_count / max(compared_count, 1))
                matched_residuals = residuals["residuals"][residuals["is_matched"]]
                if matched_residuals.size > 0:
                    mean_residual = float(np.nanmean(matched_residuals))
                    median_residual = float(np.nanmedian(matched_residuals))
                else:
                    mean_residual = 0.0
                    median_residual = 0.0
                dynamic_steps.append(
                    DynamicStepInfo(
                        frame_idx=time_idx,
                        frame_id=curr_data["frame_id"],
                        pose_source=pose_source,
                        points_total=int(source_obs["points_cam"].shape[0]),
                        matched_points=int(residuals["matched_points"]),
                        unmatched_points=int(residuals["unmatched_points"]),
                ...
                draw_dynamic_overlay(
                    last_data["im_rgb_u8"],
                    last_uv[dynamic_mask],
                    last_uv[static_mask],
                    os.path.join(dynamic_prev_dir, f"{time_idx:06d}.png"),
                    alpha=config["dynamic"].get("overlay_alpha", 0.35),
                    radius=config["dynamic"].get("overlay_radius_px", 2),
                )
                draw_dynamic_overlay(
                    curr_data["im_rgb_u8"],
                    curr_uv[dynamic_mask],
                    curr_uv[static_mask],
                    os.path.join(dynamic_curr_dir, f"{time_idx:06d}.png"),
                    alpha=config["dynamic"].get("overlay_alpha", 0.35),
                    radius=config["dynamic"].get("overlay_radius_px", 2),
                )

            if config["dynamic"].get("enable_lidar_icp", True):
                lidar_max_points = config["dynamic"].get(
                    "lidar_max_points",
                    config["tracking"].get("fused_lidar_max_points", 120000),
                )
                lidar_min_forward_m = config["dynamic"].get(
                    "lidar_min_forward_m",
                    config["tracking"].get("lidar_min_forward_m", 0.0),
                )
                lidar_max_forward_m = config["dynamic"].get(
                    "lidar_max_forward_m",
                    config["tracking"].get("lidar_max_forward_m", 0.0),
                )
                last_lidar = lidar_to_pointcloud(
                    dataset,
                    last_data["frame_id"],
                    lidar_max_points,
                    lidar_min_forward_m,
                ...
                curr_lidar = lidar_to_pointcloud(
                    dataset,
                    curr_data["frame_id"],
                    lidar_max_points,
                    lidar_min_forward_m,
                ...
                last_lidar_cam = transform_pointcloud(last_lidar, lidar_to_cam)
                curr_lidar_cam = transform_pointcloud(curr_lidar, lidar_to_cam)
                last_lidar_points = np.asarray(last_lidar_cam.points, dtype=np.float64)
                curr_lidar_points = np.asarray(curr_lidar_cam.points, dtype=np.float64)

                lidar_source_obs = build_lidar_observations(
                    last_lidar_points,
                    intrinsics.detach().cpu().numpy(),
                    np.array(last_data["im_rgb_u8"].shape[:2], dtype=np.int32),
                )
                lidar_target_obs = build_lidar_observations(
                    curr_lidar_points,
                    intrinsics.detach().cpu().numpy(),
                    np.array(curr_data["im_rgb_u8"].shape[:2], dtype=np.int32),
                )

                if (
                    lidar_source_obs["points_cam"].shape[0] > 0
                    and lidar_target_obs["points_cam"].shape[0] > 0
                ):
                    lidar_residuals = run_lidar_icp_dynamic_residuals(
                        lidar_source_obs,
                        lidar_target_obs,
                        eval_transform,
                        intrinsics.detach().cpu().numpy(),
                        np.array(curr_data["im_rgb_u8"].shape[:2], dtype=np.int32),
                        config["dynamic"].get(
                            "lidar_icp_corr_threshold",
                            config["dynamic"].get(
                                "icp_corr_threshold",
                                config["tracking"].get("icp_corr_threshold", 0.5),
                            ),
                        ),
                        config["dynamic"].get(
                            "lidar_residual_thresh_m",
                            config["dynamic"].get("residual_thresh_m", 0.5),
                        ),
                        config["dynamic"].get(
                            "lidar_unmatched_penalty",
                            config["dynamic"].get("unmatched_penalty", 2.0),
                        ),
                    )

                    lidar_compared_count = int(lidar_residuals["last_uv"].shape[0])
                    if lidar_compared_count > 0:
                        lidar_last_uv = lidar_residuals["last_uv"]
                        lidar_curr_uv = lidar_residuals["curr_uv"]
                        lidar_dynamic_mask = lidar_residuals["dynamic_mask"]
                        lidar_static_mask = lidar_residuals["static_mask"]
                        lidar_dynamic_count = int(np.sum(lidar_dynamic_mask))
                        lidar_dynamic_ratio = float(
                            lidar_dynamic_count / max(lidar_compared_count, 1)
                        )
                        lidar_matched_residuals = lidar_residuals["residuals"][
                            lidar_residuals["is_matched"]
                        ]
                        if lidar_matched_residuals.size > 0:
                            lidar_mean_residual = float(np.nanmean(lidar_matched_residuals))
                            lidar_median_residual = float(np.nanmedian(lidar_matched_residuals))
                        else:
                            lidar_mean_residual = 0.0
                            lidar_median_residual = 0.0

                        dynamic_steps_lidar.append(
                            DynamicStepInfo(
                                frame_idx=time_idx,
                                frame_id=curr_data["frame_id"],
                                pose_source=pose_source,
                                points_total=int(lidar_source_obs["points_cam"].shape[0]),
                                matched_points=int(lidar_residuals["matched_points"]),
                                unmatched_points=int(lidar_residuals["unmatched_points"]),
                                dynamic_count=lidar_dynamic_count,
                                dynamic_ratio=lidar_dynamic_ratio,
                                mean_residual_m=lidar_mean_residual,
                                median_residual_m=lidar_median_residual,
                            )
                        )
                        draw_dynamic_overlay(
                            last_data["im_rgb_u8"],
                            lidar_last_uv[lidar_dynamic_mask],
                            lidar_last_uv[lidar_static_mask],
                            os.path.join(dynamic_prev_lidar_dir, f"{time_idx:06d}.png"),
                            alpha=config["dynamic"].get("overlay_alpha", 0.35),
                            radius=config["dynamic"].get("overlay_radius_px", 2),
                        )
                        draw_dynamic_overlay(
                            curr_data["im_rgb_u8"],
                            lidar_curr_uv[lidar_dynamic_mask],
                            lidar_curr_uv[lidar_static_mask],
                            os.path.join(dynamic_curr_lidar_dir, f"{time_idx:06d}.png"),
                            alpha=config["dynamic"].get("overlay_alpha", 0.35),
                            radius=config["dynamic"].get("overlay_radius_px", 2),
                        )
    ...
    write_step_csv(os.path.join(output_dir, "per_frame_stats.csv"), dynamic_steps)
    metrics = {
        "pose_source": config["dynamic"].get("pose_source", "fused_init"),
        "source_modality": "depth_dense",
        "sequence": os.path.basename(dataset_config["sequence"]),
        "num_pairs": len(dynamic_steps),
        "mean_dynamic_ratio": float(np.mean([s.dynamic_ratio for s in dynamic_steps])) if dynamic_steps else 0.0,
        "mean_points_total": float(np.mean([s.points_total for s in dynamic_steps])) if dynamic_steps else 0.0,
        "mean_matched_points": float(np.mean([s.matched_points for s in dynamic_steps])) if dynamic_steps else 0.0,
        "mean_unmatched_points": float(np.mean([s.unmatched_points for s in dynamic_steps])) if dynamic_steps else 0.0,
        "mean_residual_m": float(np.mean([s.mean_residual_m for s in dynamic_steps])) if dynamic_steps else 0.0,
        "median_residual_m": float(np.mean([s.median_residual_m for s in dynamic_steps])) if dynamic_steps else 0.0,
    }
    ...
    if config["dynamic"].get("enable_lidar_icp", True):
        write_step_csv(os.path.join(output_dir, "per_frame_stats_lidar.csv"), dynamic_steps_lidar)
        lidar_metrics = {
            "pose_source": config["dynamic"].get("pose_source", "fused_init"),
            "source_modality": "lidar",
            "sequence": os.path.basename(dataset_config["sequence"]),
            "num_pairs": len(dynamic_steps_lidar),
            "mean_dynamic_ratio": float(np.mean([s.dynamic_ratio for s in dynamic_steps_lidar])) if dynamic_steps_lidar else 0.0,
            "mean_points_total": float(np.mean([s.points_total for s in dynamic_steps_lidar])) if dynamic_steps_lidar else 0.0,
            "mean_matched_points": float(np.mean([s.matched_points for s in dynamic_steps_lidar])) if dynamic_steps_lidar else 0.0,
            "mean_unmatched_points": float(np.mean([s.unmatched_points for s in dynamic_steps_lidar])) if dynamic_steps_lidar else 0.0,
            "mean_residual_m": float(np.mean([s.mean_residual_m for s in dynamic_steps_lidar])) if dynamic_steps_lidar else 0.0,
            "median_residual_m": float(np.mean([s.median_residual_m for s in dynamic_steps_lidar])) if dynamic_steps_lidar else 0.0,
        }
        with open(os.path.join(output_dir, "metrics_lidar.json"), "w") as f:
            json.dump(lidar_metrics, f, indent=2)
        print(json.dumps(lidar_metrics, indent=2))
...
if __name__ == "__main__":
    ...
    if args.start is not None:
        experiment.config["data"]["start"] = args.start
    if args.end is not None:
        experiment.config["data"]["end"] = args.end
    if args.stride is not None:
        experiment.config["data"]["stride"] = args.stride
    if args.max_frames is not None:
        experiment.config["data"]["num_frames"] = args.max_frames
    if args.device is not None:
        experiment.config["primary_device"] = args.device
    if args.output_dir is not None:
        experiment.config["workdir"] = args.output_dir

    sequence = os.path.basename(experiment.config["data"]["sequence"])
    start = experiment.config["data"]["start"]
    end = experiment.config["data"]["end"]
    stride = experiment.config["data"]["stride"]
    experiment.config["workdir"] = os.path.join(experiment.config["workdir"], sequence)
    experiment.config["run_name"] = f"{start}_{end}_{stride}"
