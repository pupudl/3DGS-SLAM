...
config_path="${CONFIG_PATH:-$code_path/experiments/kitti_dynamic_icp_region_copy/configs/kitti/lsgslam_dynamic_refined_pose.py}"
step="${STEP:-50}"
sequence=""
start=""
end=""
...
if [[ "$step" -le 0 ]]; then
    python "$code_path/experiments/kitti_dynamic_icp_region_copy/scripts/main_flow_dynamic.py" "$config_path" "${forward_args[@]}"
    exit 0
fi
...
if [[ "$end" -lt 0 ]]; then
    depth_dir="$code_path/data/kitti/sequences/$sequence/depth_sceneflow"
...
for ((i=start; i<=end; i+=step)); do
    ...
    output_dir="$code_path/experiments/kitti_dynamic_icp_region_copy/outputs/kitti/optimized_pose/$sequence/$run_name"
    if [[ -f "$output_dir/metrics.json" ]]; then
        echo "Skip completed chunk: $run_name"
        continue
    fi
    python "$code_path/experiments/kitti_dynamic_icp_region_copy/scripts/main_flow_dynamic.py" \
        "$config_path" \
        --sequence "$sequence" \
        --start "$chunk_start" \
        --end "$chunk_end" \
