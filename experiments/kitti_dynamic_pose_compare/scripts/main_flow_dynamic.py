...
from dynamic_utils import (
    DynamicStepInfo,
    TrackAccumulator,
    compute_dynamic_residuals,
    draw_dynamic_overlay,
    write_step_csv,
)
from frontend import estimate_pnp_from_matches
...
def rgbd_slam(config: dict):
    ...
    dynamic_steps = []
    dynamic_tracker = TrackAccumulator(match_radius_px=config["dynamic"].get("track_match_radius_px", 2.0))
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if (
            time_idx > 0
            and mkpts_cur is not None
            and mkpts_last is not None
            and dynamic_init_transform is not None
        ):
            dynamic_obs = estimate_pnp_from_matches(
                mkpts_cur,
                mkpts_last,
                last_data["depth_original"],
                last_data["intrinsics"],
            ...
            if dynamic_obs is not None:
                ...
                residuals = compute_dynamic_residuals(
                    dynamic_obs["points_last_cam"],
                    dynamic_obs["uv_curr"],
                    curr_data["depth_original"][0].detach().cpu().numpy(),
                    intrinsics.detach().cpu().numpy(),
                    eval_transform,
                    config["dynamic"].get("reproj_thresh_px", 8.0),
                    config["dynamic"].get("depth_thresh_m", 1.0),
                    config["dynamic"].get("depth_weight", 0.5),
                )

                last_uv = mkpts_last.detach().cpu().numpy()[dynamic_obs["valid_indices"]]
                curr_uv = mkpts_cur.detach().cpu().numpy()[dynamic_obs["valid_indices"]]
                dynamic_tracker.update(
                    last_uv,
                    curr_uv,
                    residuals["dynamic_mask"],
                    residuals["score"],
                )
                dynamic_count = int(np.sum(residuals["dynamic_mask"]))
                match_count = int(dynamic_obs["points_last_cam"].shape[0])
                dynamic_ratio = float(dynamic_count / max(match_count, 1))
                valid_depth = residuals["valid_depth"]
                mean_depth_err = float(np.nanmean(residuals["depth_err"][valid_depth])) if np.any(valid_depth) else 0.0
                dynamic_steps.append(
                    DynamicStepInfo(
                        ...
                        matches=match_count,
                        pnp_inliers=int(dynamic_obs["num_inliers"]),
                        pose_inlier_count=int(match_count - dynamic_count),
                        dynamic_count=dynamic_count,
                        dynamic_ratio=dynamic_ratio,
                        mean_reproj_px=float(np.mean(residuals["reproj_err"])),
                        mean_depth_err_m=mean_depth_err,
                    )
                )
                draw_dynamic_overlay(
                    last_data["im_rgb_u8"],
    ...
    dynamic_tracker.export_csv(
        os.path.join(output_dir, "dynamic_tracks.csv"),
        config["dynamic"].get("track_dynamic_ratio_thresh", 0.6),
        config["dynamic"].get("track_score_thresh", 10.0),
    )
    metrics = {
        "pose_source": config["dynamic"].get("pose_source", "fused_init"),
        "sequence": os.path.basename(dataset_config["sequence"]),
        ...
        "mean_reproj_px": float(np.mean([s.mean_reproj_px for s in dynamic_steps])) if dynamic_steps else 0.0,
        "mean_depth_err_m": float(np.mean([s.mean_depth_err_m for s in dynamic_steps])) if dynamic_steps else 0.0,
        "mean_pnp_inliers": float(np.mean([s.pnp_inliers for s in dynamic_steps])) if dynamic_steps else 0.0,
    }
