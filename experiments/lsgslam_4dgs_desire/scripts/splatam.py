...
from utils.desire_4dgs import add_initial_params, add_missing_params, is_enabled as desire_4dgs_enabled
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution, desire_4dgs_cfg=None):
    ...
    if desire_4dgs_cfg is not None:
        params = add_initial_params(params, num_pts, 0, num_frames, desire_4dgs_cfg)
...
def initialize_first_timestep(dataset, num_frames, scene_radius_depth_ratio, 
    ...
    params, variables = initialize_params(
        ...
        config.get("desire_4dgs", {}) if config is not None else None,
    )
...
def render_desire_velocity_map(params, curr_data, rendervar, transformed_gaussians):
    feature_rendervar = dict(rendervar)
    feature_rendervar["colors_precomp"] = transformed_gaussians["inst_velocity"]
    feature_rendervar["means2D"] = torch.zeros_like(
        params["means3D"], requires_grad=True, device="cuda"
    ) + 0
    v_render, _, _, _ = Renderer(raster_settings=curr_data["cam"])(**feature_rendervar)
    return v_render
...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
             ...
             grad_mask=None, render_pair_out=None, desire_4dgs_cfg=None):
    ...
    desire_v_reg = None
    if desire_4dgs_enabled(params, desire_4dgs_cfg) and mapping and render_pair_out is None:
        lambda_v_reg = float((desire_4dgs_cfg or {}).get("lambda_v_reg", 0.0))
        if lambda_v_reg > 0.0:
            loss_mult = torch.ones_like(depth, dtype=depth.dtype)
            dynamic_mask = curr_data.get("dynamic_mask")
            if dynamic_mask is not None:
                if dynamic_mask.shape[-2:] != depth.shape[-2:]:
                    dynamic_mask = F.interpolate(
                        dynamic_mask.float().unsqueeze(0),
                        size=depth.shape[-2:],
                        mode="nearest",
                    ).squeeze(0).bool()
                loss_mult = (~dynamic_mask.bool()).float()
            if sky_mask is not None:
                loss_mult = loss_mult * invert_bool_mask(sky_mask).float()
            v_render = render_desire_velocity_map(
                params,
                curr_data,
                rendervar,
                transformed_gaussians,
            )
            v_map = v_render / silhouette.unsqueeze(0).clamp_min(1e-5)
            desire_v_reg = (torch.abs(v_map) * loss_mult).mean()
            desire_v_reg = desire_v_reg * float((desire_4dgs_cfg or {}).get("velocity_loss_scale", 10.0))
    ...
    if desire_v_reg is not None:
        weighted_losses["desire_v_reg"] = desire_v_reg * float((desire_4dgs_cfg or {}).get("lambda_v_reg", 0.0))
...
def initialize_new_params(new_pt_cld, mean3_sq_dist, gaussian_distribution, time_idx=None, num_frames=None, desire_4dgs_cfg=None):
    ...
    if (
        desire_4dgs_cfg is not None
        and time_idx is not None
        and num_frames is not None
    ):
        params = add_initial_params(params, num_pts, time_idx, num_frames, desire_4dgs_cfg)
    for k, v in params.items():
        # Check if value is already a torch tensor
        if not isinstance(v, torch.Tensor):
            params[k] = torch.nn.Parameter(torch.tensor(v).cuda().float().contiguous().requires_grad_(True))
...
def add_new_gaussians(params, variables, curr_data, sil_thres, 
                      ...
                      num_frames=None, desire_4dgs_cfg=None):
...
def rgbd_slam(config: dict):
    ...
    if "desire_4dgs" not in config:
        config["desire_4dgs"] = {"enabled": False}
    desire_4dgs_cfg = config.get("desire_4dgs", {})
    print(f"{config}")
    ...
    if config['load_checkpoint']:
        ...
        params = add_missing_params(params, checkpoint_time_idx, num_frames, desire_4dgs_cfg)
        variables['max_2D_radius'] = torch.zeros(params['means3D'].shape[0]).cuda().float()
        ...
        for time_idx in range(checkpoint_time_idx):
            ...
            if time_idx in keyframe_time_indices:
                ...
                dynamic_mask = dynamic_mask_manager.load_dynamic_mask_for_frame(
                    time_idx,
                    num_frames,
                    color.shape[1],
                    color.shape[2],
                )
                curr_keyframe = {'id': time_idx, 'est_w2c': curr_w2c, 'color': color, 'depth': depth, 'sky_mask': sky_mask, 'dynamic_mask': dynamic_mask}
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        dynamic_record = dynamic_mask_manager.run_for_frame(
            time_idx=time_idx,
            num_frames=num_frames,
            params=params,
            sky_mask=sky_mask,
        )
        loaded_dynamic_mask = dynamic_mask_manager.load_dynamic_mask(
            dynamic_record,
            color.shape[1],
            color.shape[2],
        )
        if loaded_dynamic_mask is not None:
            curr_data["dynamic_mask"] = loaded_dynamic_mask
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            if config['mapping']['add_new_gaussians'] and time_idx > 0:
                ...
                params, variables = add_new_gaussians(params, variables, densify_curr_data, 
                                                      ...
                                                      num_frames=num_frames, desire_4dgs_cfg=desire_4dgs_cfg)
        ...
        dynamic_finalize_record = dynamic_mask_manager.finalize_for_frame(
            time_idx=time_idx,
            num_frames=num_frames,
            params=params,
            curr_data=curr_data,
        ...
        finalized_dynamic_mask = dynamic_mask_manager.load_dynamic_mask(
            dynamic_finalize_record,
            color.shape[1],
            color.shape[2],
        )
        if finalized_dynamic_mask is not None:
            curr_data["dynamic_mask"] = finalized_dynamic_mask
        ...
        if ((time_idx == 0) or ((time_idx+1) % config['keyframe_every'] == 0) or \
            ...
            with torch.no_grad():
                ...
                curr_w2c[:3, 3] = curr_cam_tran
                # Initialize Keyframe Info
                curr_keyframe = {'id': time_idx, 'est_w2c': curr_w2c, 'color': color, 'depth': depth, 'sky_mask': sky_mask, 'dynamic_mask': curr_data.get("dynamic_mask")}
                # Add to keyframe list
                keyframe_list.append(curr_keyframe)
