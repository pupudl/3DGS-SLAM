...
DATA_ROOT = "/home/qiuyu/data/Projects/LSG-SLAM/data"
...
def parse_args():
    ...
    parser.add_argument(
        "--base_folder",
        default="results/kitti360-0000-pnp-fused-icp-all",
    ...
    parser.add_argument(
        "--kitti_base_folder",
        default=os.path.join(DATA_ROOT, "kitti", "sequences"),
...
if __name__ == "__main__":
    ...
    if len(loop_res_folders) == 0:
        raise RuntimeError(
            f"No loop result folder found in {base_folder}. "
            f"Expected a folder name containing 'loop' (e.g. '*_loops')."
        )
    if len(loop_res_folders) > 1:
        # Choose the most recently modified loop folder for convenience.
        loop_res_folders = sorted(
            loop_res_folders,
            key=lambda x: os.path.getmtime(os.path.join(base_folder, x)),
            reverse=True,
        )
        print("Multiple loop result folders found, use latest:", loop_res_folders[0])
