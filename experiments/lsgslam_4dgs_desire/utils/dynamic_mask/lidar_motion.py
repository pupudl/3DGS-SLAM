...
class LidarMotionProbe:
    ...
    def save_pair(self, output_root, dataset, params, prev_time_idx, curr_time_idx, device):
        ...
        try:
            ...
            nonground_summary = {}
            if self.cfg.get(
                "compute_nonground_residual",
                self.cfg.get("save_nonground_visualizations", True),
            ):
                (
                    prev_nonground,
                    curr_nonground,
    ...
    def _get_projection_calib(self):
        ...
        data_root = os.environ.get("LSGSLAM_DATA_ROOT", "/home/qiuyu/data/Projects/LSG-SLAM/data")
        perspective_path = os.path.join(data_root, "kitti360", "calibration", "perspective.txt")
    ...
    def _save_bev_feature_residual(
        ...
        if (
            mask_filename
            and self.cfg.get("save_feature_mask_visualization", True)
            and self._save_visualizations()
        ):
