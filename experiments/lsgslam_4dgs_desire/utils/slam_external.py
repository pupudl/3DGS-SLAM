...
from utils.desire_4dgs import get_inst_velocity, get_scaling_t, is_enabled as desire_4dgs_enabled
...
def densify(params, variables, optimizer, iter, densify_dict, desire_4dgs_cfg=None):
    if iter <= densify_dict['stop_after']:
        ...
        if (iter >= densify_dict['start_after']) and (iter % densify_dict['densify_every'] == 0):
            ...
            if desire_4dgs_enabled(params, desire_4dgs_cfg):
                decay_factor = n * 0.8
                stds_t = get_scaling_t(params)[to_split].repeat(n, 1)
                means_t = torch.zeros((stds_t.size(0), 1), device="cuda")
                samples_t = torch.normal(mean=means_t, std=stds_t)
                new_params['_t'] = samples_t + params['_t'][to_split].repeat(n, 1)
                new_params['_scaling_t'] = torch.log(stds_t / decay_factor)
                new_params['_velocity'] = params['_velocity'][to_split].repeat(n, 1)
                new_params['means3D'] += get_inst_velocity(params, desire_4dgs_cfg)[to_split].repeat(n, 1) * samples_t
