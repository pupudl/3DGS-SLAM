...
from utils.desire_4dgs import get_dynamic_state, is_enabled as desire_4dgs_enabled
...
def transformed_params2rendervar(params, transformed_gaussians):
    ...
    if params['log_scales'].shape[1] == 1:
        ...
        log_scales = params['log_scales']
    opacities = torch.sigmoid(params['logit_opacities'])
    if 'marginal_t' in transformed_gaussians:
        opacities = opacities * transformed_gaussians['marginal_t']
    # Initialize Render Variables
    rendervar = {
...
def transform_to_frame(params, time_idx, gaussians_grad, camera_grad, desire_4dgs_cfg=None):
    """
    ...
    dynamic_state = None
    if desire_4dgs_enabled(params, desire_4dgs_cfg):
        dynamic_state = get_dynamic_state(
            params,
            time_idx,
            params['cam_trans'].shape[-1],
            desire_4dgs_cfg,
            gaussians_grad=gaussians_grad,
        )
        pts = dynamic_state['means3D']
    elif gaussians_grad:
        pts = params['means3D']
    ...
    if dynamic_state is not None:
        transformed_gaussians['marginal_t'] = dynamic_state['marginal_t']
        transformed_gaussians['scaling_t'] = dynamic_state['scaling_t']
        transformed_gaussians['inst_velocity'] = dynamic_state['inst_velocity']
