...
from utils.slam_helpers import (
    params2rendervar, transformed_params2rendervar, transformed_params2depthplussilhouette,
    ...
    for k, v in params.items():
        ...
        if not isinstance(v, torch.Tensor):
            params[k] = torch.nn.Parameter(
                torch.tensor(np.asarray(v), dtype=torch.float32, device="cuda")
                .contiguous()
                .requires_grad_(True)
            )
        else:
...
def prune_gaussians_by_id(params, variables, optimizer, gaussian_ids):
    if gaussian_ids is None or len(gaussian_ids) == 0:
        return params, variables
    device = params["means3D"].device
    num_pts = params["means3D"].shape[0]
    valid_ids = [int(gaussian_id) for gaussian_id in gaussian_ids if 0 <= int(gaussian_id) < num_pts]
    if not valid_ids:
        return params, variables
    to_remove = torch.zeros(num_pts, device=device, dtype=torch.bool)
    to_remove[torch.as_tensor(valid_ids, device=device, dtype=torch.long)] = True
    ...
    if render_pair_out is not None:
        ...
        render_pair_out["gaussian_data"] = {
            ...
            "gaussian_ids": torch.arange(
                params["means3D"].shape[0], device=params["means3D"].device, dtype=torch.long
            ).detach().cpu(),
            "depth": transformed_gaussians["means3D"][:, 2].detach().cpu(),
            "attribution_mask": attribution_mask.detach().cpu(),
            "timestep": variables.get("timestep", None).detach().cpu()
            if variables.get("timestep", None) is not None
            else None,
        }
...
def render_rgb_for_frame(params, curr_data, iter_time_idx):
    curr_cam_rot = F.normalize(params['cam_unnorm_rots'][..., iter_time_idx].detach())
    ...
    image_h = curr_data["im"].shape[1]
    image_w = curr_data["im"].shape[2]
    render_cam = setup_camera(
        image_w,
        image_h,
        intrinsics.detach().cpu().numpy(),
        curr_w2c.detach().cpu().numpy(),
        depth_threshold=curr_data.get("depth_threshold"),
    )

    rendervar = params2rendervar(params)
    image, _, _, _ = Renderer(raster_settings=render_cam)(**rendervar)
    return image


def render_selected_gaussians_for_frame(params, curr_data, iter_time_idx, gaussian_ids):
    if gaussian_ids is None:
        return None
    if isinstance(gaussian_ids, torch.Tensor):
        selected_ids = gaussian_ids.detach().to(device=params["means3D"].device, dtype=torch.long)
    else:
    ...
    if selected_ids.numel() == 0:
        return None
    selected_ids = torch.unique(selected_ids)
    ...
    for key, value in params.items():
        if key in ["cam_unnorm_rots", "cam_trans"]:
            subset_params[key] = value
        else:
            subset_params[key] = value[selected_ids]
    subset_transformed = {
        key: value[selected_ids] for key, value in transformed_gaussians.items()
    }
    rendervar = transformed_params2rendervar(subset_params, subset_transformed)
    image, _, _, _ = Renderer(raster_settings=curr_data["cam"])(**rendervar)

    sky_mask = curr_data.get("sky_mask")
    if sky_mask is not None:
        color_keep_mask = torch.tile(invert_bool_mask(sky_mask), (3, 1, 1)).detach()
        image = image * color_keep_mask
    return image.detach().cpu()


def save_rgb_tensor_image(path, image):
    if image is None:
        return
    image_np = image.detach().float().cpu().clamp(0.0, 1.0).permute(1, 2, 0).numpy()
    image_uint8 = (image_np * 255.0).astype(np.uint8)
    cv2.imwrite(path, cv2.cvtColor(image_uint8, cv2.COLOR_RGB2BGR))


def _save_single_channel_image(path, array):
...
def save_probe_debug(frame_dir, render_pair, params, time_idx):
    os.makedirs(frame_dir, exist_ok=True)

    render_image = render_pair["render_image"].float().cpu().numpy()
    gt_image = render_pair["gt_image"].float().cpu().numpy()
    render_depth = render_pair["render_depth"].float().cpu().numpy()
    gt_depth = render_pair["gt_depth"].float().cpu().numpy()
    presence_sil_mask = render_pair["presence_sil_mask"].float().cpu().numpy()
    valid_depth_mask = render_pair["valid_depth_mask"].float().cpu().numpy()
    loss_mask = render_pair["loss_mask"].float().cpu().numpy()

    render_rgb_uint8 = np.clip(render_image.transpose(1, 2, 0), 0.0, 1.0)
    render_rgb_uint8 = (render_rgb_uint8 * 255.0).astype(np.uint8)
    dark_frac = float((render_rgb_uint8 < 5).mean())
    render_mean = float(render_rgb_uint8.mean())
    presence_ratio = float(presence_sil_mask.mean())
    valid_depth_ratio = float(valid_depth_mask.mean())
    loss_mask_ratio = float(loss_mask.mean())

    curr_cam_rot = F.normalize(params['cam_unnorm_rots'][..., time_idx].detach()).squeeze(0)
    curr_cam_tran = params['cam_trans'][..., time_idx].detach().squeeze(0)

    stats_lines = [
        f"render_mean={render_mean:.6f}",
        f"render_min={int(render_rgb_uint8.min())}",
        f"render_max={int(render_rgb_uint8.max())}",
        f"dark_frac_lt5={dark_frac:.6f}",
        f"presence_sil_ratio={presence_ratio:.6f}",
        f"valid_depth_ratio={valid_depth_ratio:.6f}",
        f"loss_mask_ratio={loss_mask_ratio:.6f}",
        f"render_depth_valid_mean={float(render_depth[valid_depth_mask > 0].mean()) if np.any(valid_depth_mask > 0) else float('nan'):.6f}",
        f"gt_depth_valid_mean={float(gt_depth[valid_depth_mask > 0].mean()) if np.any(valid_depth_mask > 0) else float('nan'):.6f}",
        "cam_quat_wxyz=" + " ".join(f"{v:.8f}" for v in curr_cam_rot.cpu().numpy().tolist()),
        "cam_trans_xyz=" + " ".join(f"{v:.8f}" for v in curr_cam_tran.cpu().numpy().tolist()),
    ]
    with open(os.path.join(frame_dir, "probe_stats.txt"), "w", encoding="ascii") as f:
        f.write("\n".join(stats_lines) + "\n")

    if render_mean < 1.0 or dark_frac > 0.99:
        sil_uint8 = (presence_sil_mask * 255.0).astype(np.uint8)
        valid_uint8 = (valid_depth_mask * 255.0).astype(np.uint8)
        loss_uint8 = (loss_mask * 255.0).astype(np.uint8)
        depth_valid = gt_depth[valid_depth_mask > 0]
        if depth_valid.size > 0:
            depth_max = float(depth_valid.max())
        else:
            depth_max = 1.0
        depth_max = max(depth_max, 1e-6)
        render_depth_vis = np.clip(render_depth[0] / depth_max, 0.0, 1.0) * 255.0
        gt_depth_vis = np.clip(gt_depth[0] / depth_max, 0.0, 1.0) * 255.0
        _save_single_channel_image(os.path.join(frame_dir, "presence_sil_mask.png"), sil_uint8)
        _save_single_channel_image(os.path.join(frame_dir, "valid_depth_mask.png"), valid_uint8)
        _save_single_channel_image(os.path.join(frame_dir, "loss_mask.png"), loss_uint8)
        _save_single_channel_image(os.path.join(frame_dir, "render_depth_vis.png"), render_depth_vis)
        _save_single_channel_image(os.path.join(frame_dir, "gt_depth_vis.png"), gt_depth_vis)
...
def rgbd_slam(config: dict):
    ...
    if "stage1_feature_probe" not in config:
        config["stage1_feature_probe"] = {"enabled": False}
    if "rigidmask_frontend_probe" not in config:
        config["rigidmask_frontend_probe"] = {"enabled": False}
    if "lidar_motion_probe" not in config:
        config["lidar_motion_probe"] = {"enabled": False}
    print(f"{config}")
    ...
    rigidmask_frontend_probe_cfg = config.get("rigidmask_frontend_probe", {})
    lidar_motion_probe_cfg = config.get("lidar_motion_probe", {})
    feature_probe_output_dir = None
    depth_probe_output_dir = None
    rigidmask_frontend_probe_output_dir = None
    rigidmask_frontend_probe = None
    lidar_motion_probe_output_dir = None
    lidar_motion_probe = None
    ...
    if depth_probe_cfg.get("enabled", False):
        depth_probe_output_dir = os.path.join(
            output_dir,
            depth_probe_cfg.get("output_subdir", "depth_probe"),
        )
    ...
    if feature_probe_cfg.get("enabled", False):
        ...
        os.makedirs(feature_probe_output_dir, exist_ok=True)
        feature_probe = Stage1FeatureProbe(
            checkpoint_path=feature_probe_cfg["checkpoint_path"],
            device=device,
            model_name=feature_probe_cfg.get(
                "model_name", "vit_small_patch14_reg4_dinov2.lvd142m"
            ),
            save_raw_tensors=feature_probe_cfg.get("save_raw_tensors", True),
            save_visualizations=feature_probe_cfg.get("save_visualizations", True),
            save_input_rgbs=feature_probe_cfg.get("save_input_rgbs", False),
            save_gaussian_anomaly_scores=feature_probe_cfg.get(
                "save_gaussian_anomaly_scores", True
            ),
            gaussian_anomaly_topk=feature_probe_cfg.get("gaussian_anomaly_topk", 64),
            gaussian_anomaly_radius_scale=feature_probe_cfg.get(
                "gaussian_anomaly_radius_scale", 1.5
            ),
            gaussian_anomaly_min_valid_pixels=feature_probe_cfg.get(
                "gaussian_anomaly_min_valid_pixels", 4
            ),
            gaussian_anomaly_use_distance_weight=feature_probe_cfg.get(
                "gaussian_anomaly_use_distance_weight", True
            ),
            gaussian_anomaly_threshold=feature_probe_cfg.get(
                "gaussian_anomaly_threshold", 0.35
            ),
            gaussian_anomaly_min_component_pixels=feature_probe_cfg.get(
        ...
        save_prune_summary = feature_probe_cfg.get("save_prune_summary", True)
        save_pruned_map_render = feature_probe_cfg.get("save_pruned_map_render", True)
    ...
    if rigidmask_frontend_probe_cfg.get("enabled", False):
        rigidmask_frontend_probe_output_dir = os.path.join(
            output_dir,
            rigidmask_frontend_probe_cfg.get("output_subdir", "rigidmask_frontend_probe"),
        )
        os.makedirs(rigidmask_frontend_probe_output_dir, exist_ok=True)
        rigidmask_frontend_probe = RigidMaskFrontendProbe(
    ...
    if lidar_motion_probe_cfg.get("enabled", False):
        lidar_motion_probe_output_dir = os.path.join(
            output_dir,
            lidar_motion_probe_cfg.get("output_subdir", "lidar_motion_probe"),
        )
        os.makedirs(lidar_motion_probe_output_dir, exist_ok=True)
        lidar_motion_probe = LidarMotionProbe(lidar_motion_probe_cfg, PROJECT_ROOT)
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        print(f"[debug] frame {time_idx}: building Open3D point cloud with {X.shape[1]} points")
        points_np = np.ascontiguousarray(X.t().detach().cpu().numpy(), dtype=np.float64)
        colors_np = np.ascontiguousarray(color_tmp.detach().cpu().numpy(), dtype=np.float64)
        pc = o3d.geometry.PointCloud()
        pc.points = o3d.utility.Vector3dVector(points_np)
        pc.colors = o3d.utility.Vector3dVector(colors_np)
        print(f"[debug] frame {time_idx}: Open3D point cloud ready")
        ...
        if lidar_motion_probe is not None and lidar_motion_probe.should_run(time_idx):
            probe_summary = lidar_motion_probe.save_pair(
                output_root=lidar_motion_probe_output_dir,
                dataset=dataset,
                params=params,
            ...
            if probe_summary.get("status") != "ok":
                print(
                    f"LiDAR motion probe skipped at frame {time_idx}: "
                    f"{probe_summary.get('reason', 'unknown reason')}"
                )

        if rigidmask_frontend_probe is not None and rigidmask_frontend_probe.should_run(time_idx, num_frames):
            next_frame_id = get_dataset_frame_id(dataset, time_idx + 1)
            probe_summary = rigidmask_frontend_probe.save_pair(
                output_root=rigidmask_frontend_probe_output_dir,
                time_idx=time_idx,
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            for iter in range(num_iters_mapping):
                ...
                with torch.no_grad():
                    ...
                    # Prune Gaussians after densification; densify consumes the
                    # current iteration's `seen` mask and 2D gradients.
            ...
            if config.get('opt_current_frame', False):
                print('opt_current_frame')
                current_mapping_lrs = {
                                    "means3D": 0.0000,
                                    "rgb_colors": config['mapping']['lrs']['rgb_colors'],
                                    "unnorm_rotations": 0.0000,
                                    "logit_opacities": 0.0000,
                                    "log_scales": 0.0000,
                                    "cam_unnorm_rots": 0.0000,
                                    "cam_trans": 0.0000
                                    }
                optimizer = initialize_optimizer(params, current_mapping_lrs, tracking=False)
                num_iters_current_mapping = int(num_iters_mapping / 2)
                if num_iters_current_mapping > 0:
                    progress_bar = tqdm(range(num_iters_current_mapping), desc=f"Current Frame Mapping Time Step: {time_idx}")
                for iter in range(num_iters_current_mapping):
                    iter_start_time = time.time()
                    iter_time_idx = time_idx
                    iter_color = color
                    iter_depth = depth
            ...
            if (
                ...
                or depth_probe_cfg.get("enabled", False)
            ):
                with torch.no_grad():
                    render_pair = {}
                    frame_id = curr_data.get("frame_id", time_idx)
                    frame_name = f"{time_idx:06d}_frame_{frame_id}"
                    _, _, _ = get_loss(
                        params,
                        curr_data,
                        variables,
                    ...
                    save_probe_debug(
                        os.path.join(
                            (
                                feature_probe_output_dir
                                if feature_probe is not None
                                else depth_probe_output_dir
                                if depth_probe_output_dir is not None
                                else rigidmask_frontend_probe_output_dir
                            ),
                            frame_name,
                            "probe_debug",
                        ),
    ...
    with torch.no_grad():
        if config['use_wandb']:
            eval(dataset, params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                 ...
                 eval_every=config['eval_every'], save_frames=feature_probe is not None)
        else:
            eval(dataset, params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
                 mapping_iters=config['mapping']['num_iters'], add_new_gaussians=config['mapping']['add_new_gaussians'],
                 eval_every=config['eval_every'], save_frames=feature_probe is not None)
