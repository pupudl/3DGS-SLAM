...
from utils.common_utils import params2cpu, save_params, save_params_ckpt, seed_everything
...
from utils.support_filter import (
    extend_support_counts,
    filter_params_by_support_count,
    initialize_support_counts,
    update_support_counts,
)
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution):
    ...
    variables = {'max_2D_radius': torch.zeros(params['means3D'].shape[0]).cuda().float(),
                 ...
                 'support_count': initialize_support_counts(params['means3D'].shape[0])}
...
def initialize_first_timestep(dataset, num_frames, scene_radius_depth_ratio, 
    ...
    if densify_dataset is not None:
        ...
        color = color.permute(2, 0, 1) / 255 # (H, W, C) -> (C, H, W)
        depth = depth.permute(2, 0, 1) # (H, W, C) -> (C, H, W)
        densify_sky_mask = resize_bool_mask(sky_mask, color.shape[1], color.shape[2])
    ...
    scene_depth = apply_sky_mask_to_depth(depth, sky_mask)
...
def add_new_gaussians(params, variables, curr_data, sil_thres, 
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        variables = extend_support_counts(variables, new_pt_cld.shape[0])

    return params, variables


def accumulate_frame_gaussian_support(params, variables, curr_data, time_idx, support_filter_cfg):
    if (not support_filter_cfg['enabled']) or params['means3D'].shape[0] == 0:
        return variables, None
    valid_pixels = int((curr_data['depth'] > 0).sum().item())
    if valid_pixels == 0:
        return variables, {'valid_pixels': 0, 'consistent_matches': 0}
    ...
    variables, consistent_matches = update_support_counts(
        params,
        variables,
        curr_data,
        transformed_gaussians,
        support_filter_cfg['color_threshold'],
        support_filter_cfg.get('depth_tolerance', 1.0),
    )
    return variables, {
...
def attach_metadata_for_save(params_to_store, variables, intrinsics, first_frame_w2c, dataset_config,
                             gt_w2c_all_frames, keyframe_time_indices, support_keep_mask=None):
    if support_keep_mask is None:
        params_to_store['timestep'] = variables['timestep']
        params_to_store['support_count'] = variables['support_count']
    else:
        params_to_store['timestep'] = variables['timestep'][support_keep_mask]
        params_to_store['support_count'] = variables['support_count'][support_keep_mask]
        params_to_store['support_keep_mask'] = support_keep_mask

    params_to_store['intrinsics'] = intrinsics.detach().cpu().numpy()
    params_to_store['w2c'] = first_frame_w2c.detach().cpu().numpy()
    params_to_store['org_width'] = dataset_config["desired_image_width"]
    params_to_store['org_height'] = dataset_config["desired_image_height"]
    params_to_store['gt_w2c_all_frames'] = []
    for gt_w2c_tensor in gt_w2c_all_frames:
        params_to_store['gt_w2c_all_frames'].append(gt_w2c_tensor.detach().cpu().numpy())
    params_to_store['gt_w2c_all_frames'] = np.stack(params_to_store['gt_w2c_all_frames'], axis=0)
    params_to_store['keyframe_time_indices'] = np.array(keyframe_time_indices)
    return params_to_store
...
def rgbd_slam(config: dict):
    # Print Config
    print("Loaded Config:")
    if "support_filter" not in config:
        config["support_filter"] = dict(
            enabled=False,
    ...
    sky_mask_cfg = config.get("sky_mask", {})
    ...
    # exit()
    first_color_frame, _, _, _, depth_original_first_frame, _ = dataset[0]
    first_color_frame = first_color_frame.permute(2, 0, 1) / 255
    depth_original_first_frame = depth_original_first_frame.permute(2, 0, 1)
    first_sky_mask = sky_mask_predictor.get_mask(0, first_color_frame)
    threshold_depth = apply_sky_mask_to_depth(depth_original_first_frame, first_sky_mask)
    valid_threshold_depth = threshold_depth[threshold_depth > 0]
    if valid_threshold_depth.numel() == 0:
        valid_threshold_depth = depth_original_first_frame[depth_original_first_frame > 0]
    ...
    if config['load_checkpoint']:
        ...
        variables['support_count'] = initialize_support_counts(params['means3D'].shape[0])
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if time_idx == 0 or (time_idx+1) % config['map_every'] == 0:
            ...
            if config['support_filter']['enabled']:
                with torch.no_grad():
                    variables, support_stats = accumulate_frame_gaussian_support(
                        params, variables, curr_data, time_idx, config['support_filter']
                    )
                if support_stats is not None:
                    print(
                        f"Support Count Update @ Frame {time_idx}: "
                        f"{support_stats['consistent_matches']} / {support_stats['valid_pixels']} pixels matched"
                    )
                    if config['use_wandb']:
                        wandb_run.log({
                            "SupportFilter/Consistent Matches": support_stats['consistent_matches'],
                            "SupportFilter/Valid Pixels": support_stats['valid_pixels'],
                            "SupportFilter/step": wandb_time_step,
                        })
    ...
    final_eval_params = params
    support_keep_mask = None
    if config['support_filter']['enabled']:
        final_eval_params, support_keep_mask = filter_params_by_support_count(
            params, variables['support_count'], config['support_filter']['min_count']
        )
        num_total_gaussians = int(params['means3D'].shape[0])
        num_kept_gaussians = int(support_keep_mask.sum().item())
        print(
            f"Support Filter Kept {num_kept_gaussians} / {num_total_gaussians} gaussians "
            f"(min_count={config['support_filter']['min_count']})"
        )
        if config['use_wandb']:
            wandb_run.log({
                "SupportFilter/Final Kept Gaussians": num_kept_gaussians,
                "SupportFilter/Final Total Gaussians": num_total_gaussians,
                "SupportFilter/step": 1,
            })

    # Evaluate Final Parameters
    with torch.no_grad():
        if config['use_wandb']:
            eval(dataset, final_eval_params, num_frames, eval_dir, sil_thres=config['mapping']['sil_thres'],
    ...
    raw_params_to_save = convert_params_to_store(params)
    raw_params_to_save = attach_metadata_for_save(
        raw_params_to_save,
        variables,
        intrinsics,
        first_frame_w2c,
    ...
    np.savez(os.path.join(output_dir, "params_full.npz"), **params2cpu(raw_params_to_save))

    filtered_params_to_save = convert_params_to_store(final_eval_params)
    filtered_params_to_save = attach_metadata_for_save(
        filtered_params_to_save,
        variables,
        intrinsics,
        first_frame_w2c,
        dataset_config,
        gt_w2c_all_frames,
        keyframe_time_indices,
        support_keep_mask=support_keep_mask,
    )

    # Save filtered parameters as the default experiment output.
    save_params(filtered_params_to_save, output_dir)
