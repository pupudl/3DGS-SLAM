...
def remove_points(to_remove, params, variables, optimizer):
    ...
    if 'support_count' in variables.keys():
        variables['support_count'] = variables['support_count'][to_keep]
...
def densify(params, variables, optimizer, iter, densify_dict):
    if iter <= densify_dict['stop_after']:
        ...
        if (iter >= densify_dict['start_after']) and (iter % densify_dict['densify_every'] == 0):
            ...
            if 'support_count' in variables:
                variables['support_count'] = torch.cat((
                    variables['support_count'],
                    torch.zeros_like(variables['support_count'][to_clone]),
                ))
            ...
            if 'timestep' in variables:
                ...
                variables['timestep'] = torch.cat((variables['timestep'], duplicated_values))
            if 'support_count' in variables:
                num_split_new = int((n * to_split.sum()).item())
                variables['support_count'] = torch.cat((
                    variables['support_count'],
                    torch.zeros(num_split_new, dtype=variables['support_count'].dtype, device="cuda"),
                ))
