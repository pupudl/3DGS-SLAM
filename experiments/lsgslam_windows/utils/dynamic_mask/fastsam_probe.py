...
class FastSAMProbe:
    ...
    def save_for_image(
        ...
        """Run FastSAM for one image and save reusable per-frame candidates."""
        ...
        np.savez_compressed(output_npz, **npz_kwargs)

        if self.cfg.get("save_visualization", True) and output_visualization is not None:
            output_visualization = Path(output_visualization)
            output_visualization.parent.mkdir(parents=True, exist_ok=True)
            union = masks.max(axis=0).astype(np.float32) if masks.shape[0] > 0 else np.zeros(image_rgb.shape[:2], dtype=np.float32)
            cv2.imwrite(str(output_visualization), (union * 255).astype(np.uint8))
    ...
    def save_for_pair_dir(self, pair_dir, time_idx=None, frame_id=None, force=False):
        ...
        output_visualization = pair_output_path(
            pair_dir,
            "fastsam_instances.png",
            organize_outputs=False,
        )
        result = self.save_for_image(
