...
from utils.slam_external import build_rotation, calc_psnr, calc_psnr_masked
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
         ...
         eval_every=1, save_frames=False, align_traj=True, sky_mask_provider=None):
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        depth_sil, _, _, _ = Renderer(raster_settings=curr_data['cam'])(**depth_sil_rendervar)
        rastered_depth = depth_sil[0, :, :].unsqueeze(0)
        # Evaluate only valid, non-sky pixels. Multiplying invalid pixels by
        # zero and then averaging over the whole image would inflate PSNR.
        valid_depth_mask = (curr_data['depth'] > 0)
        sky_mask = None
        if sky_mask_provider is not None:
            sky_mask = sky_mask_provider.get_mask(time_idx, color)
            if sky_mask is not None:
                if sky_mask.shape[-2:] != valid_depth_mask.shape[-2:]:
                    sky_mask = F.interpolate(
                        sky_mask.float().unsqueeze(0),
                        size=valid_depth_mask.shape[-2:],
                        mode="nearest",
                    )[0] > 0.5
                valid_depth_mask = valid_depth_mask & (~sky_mask.bool())
        ...
        im, radius, _, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar)
        if mapping_iters==0 and not add_new_gaussians:
            rgb_eval_mask = valid_depth_mask & presence_sil_mask.unsqueeze(0)
        else:
            rgb_eval_mask = valid_depth_mask
        if not bool(rgb_eval_mask.any()):
            print(
                f"No valid non-sky RGB pixels at time step {time_idx}; "
                "falling back to valid-depth pixels."
            )
            rgb_eval_mask = (curr_data['depth'] > 0)
            if mapping_iters==0 and not add_new_gaussians:
                rgb_eval_mask = rgb_eval_mask & presence_sil_mask.unsqueeze(0)
        weighted_im = im * rgb_eval_mask
        weighted_gt_im = curr_data['im'] * rgb_eval_mask
        psnr = calc_psnr_masked(im, curr_data['im'], rgb_eval_mask).mean()
        valid_rgb_ratio_list.append(
            float(rgb_eval_mask.float().mean().detach().cpu().item())
        )
    ...
    valid_rgb_ratio_list = np.array(valid_rgb_ratio_list)
    avg_psnr = psnr_list.mean()
    avg_rmse = rmse_list.mean()
    avg_l1 = l1_list.mean()
    ...
    print("Average Valid Non-Sky RGB Ratio: {:.3f}".format(valid_rgb_ratio_list.mean()))
    ...
    if wandb_run is not None:
        wandb_run.log({"Final Stats/Average PSNR": avg_psnr, 
                       ...
                       "Final Stats/Average Valid Non-Sky RGB Ratio": valid_rgb_ratio_list.mean(),
                       "Final Stats/step": 1})
    ...
    np.savetxt(os.path.join(eval_dir, "valid_rgb_ratio.txt"), valid_rgb_ratio_list)

    # Plot PSNR & L1 as line plots
    fig, axs = plt.subplots(1, 2, figsize=(12, 4))
    axs[0].plot(np.arange(len(psnr_list)), psnr_list)
    axs[0].set_title("RGB PSNR (Valid Non-Sky Pixels)")
    axs[0].set_xlabel("Time Step")
    axs[0].set_ylabel("PSNR")
