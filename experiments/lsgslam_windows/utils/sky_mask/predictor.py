...
class SkyMaskPredictor:
    def __init__(
        ...
        self.precomputed_root = self.config.get("precomputed_root", "")
        self.precomputed_subdir = self.config.get("precomputed_subdir", "")
        ...
        self._warned_mmseg_failure = False
        self._model = None
    ...
    def get_mask(self, time_idx: int, color: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        return torch.tensor(mask[None, ...], device=self.device, dtype=torch.bool)
    ...
    def _predict_with_mmseg(self, time_idx: int, color: torch.Tensor) -> np.ndarray:
        ...
        try:
            ...
            if not self._warned_mmseg_failure:
                print(
                    "Sky mask mmseg inference failed; falling back to an empty sky mask. "
                    f"Reason: {exc}"
                )
                self._warned_mmseg_failure = True
    ...
    def _get_precomputed_mask_path(self, time_idx: int) -> Optional[str]:
        ...
        if self.precomputed_subdir:
            sequence_root = self.precomputed_root or getattr(self.dataset, "input_folder", "")
            if not sequence_root:
                return None
            return os.path.join(
                sequence_root,
                self.precomputed_subdir,
        ...
        rel_path = self._safe_image_relpath(image_path)
    ...
    def _get_cache_path(self, time_idx: int) -> Optional[str]:
        ...
        rel_path = self._safe_image_relpath(image_path, fallback=f"{time_idx:06d}.png")
    ...
    def _safe_image_relpath(self, image_path: str, fallback: Optional[str] = None) -> str:
        fallback = fallback or os.path.basename(image_path)
        rel_path = None
        if self.dataset_basedir:
            try:
                image_real = os.path.realpath(os.path.abspath(image_path))
                base_real = os.path.realpath(os.path.abspath(self.dataset_basedir))
                if os.path.commonpath([image_real, base_real]) == base_real:
                    rel_path = os.path.relpath(image_real, base_real)
        ...
        if not parts:
            return os.path.basename(fallback) or "mask.png"
