...
def calc_psnr_masked(img1, img2, valid_mask):
    """Calculate per-channel PSNR using only pixels selected by valid_mask."""
    if img1.shape != img2.shape:
        raise ValueError("img1 and img2 must have identical shapes")
    if valid_mask.ndim == 3:
        if valid_mask.shape[0] != 1:
            raise ValueError("valid_mask must have shape [H, W] or [1, H, W]")
        valid_mask = valid_mask[0]
    if valid_mask.ndim != 2 or valid_mask.shape != img1.shape[-2:]:
        raise ValueError("valid_mask must match the image spatial dimensions")
    valid_mask = valid_mask.to(device=img1.device, dtype=torch.bool)
    if not bool(valid_mask.any()):
        raise ValueError("valid_mask contains no valid pixels")
    squared_error = (img1 - img2).pow(2)[:, valid_mask]
    mse = squared_error.mean(dim=1, keepdim=True)
    return 20 * torch.log10(1.0 / torch.sqrt(mse))
...
def densify(params, variables, optimizer, iter, densify_dict, accumulate_grad=True):
